...
export default function QuantitySoldDashboard() {
  ...
  const calculateStats = () => {
    if (data.length === 0)
      return { totalQuantity: 0, totalItems: 0, avgQuantity: 0, topItem: "-" };
      ...
      return (
        ...
        >
          ...
          <p style={{ margin: "4px 0", color: "#8884d8", fontSize: "14px" }}>
            Quantity Sold:{" "}
            <strong>{data.total_quantity_sold?.toLocaleString()}</strong>
          </p>
          {data.sales_year && (
      ...
      {!loading && data.length > 0 && (
        ...
        >
          ...
          >
            ...
            >
              {viewType === "category" || viewType === "category_year"
                ? "Total Categories"
                : "Total Products"}
            </h4>
            <p style={{ fontSize: "32px", fontWeight: "bold", margin: 0 }}>
      ...
      >
        ...
        <button
          ...
          style={{
            ...
            transition: "all 0.3s ease",
            transform:
              viewType === "category_year" ? "translateY(-2px)" : "none",
          }}
          onMouseEnter={(e) => {
            if (viewType !== "category_year") {
              e.target.style.backgroundColor = "#e8e8e8";
